...
// Reemplazamos el mock por una función real con fetch
const obtenerPrimerTramoDeApi = async (programa, pdfFile) => {
  try {
    // 1. Crear el FormData
    const formData = new FormData();
    
    // 2. Agregar el texto (con la clave 'message' que espera tu Django Serializer)
    if (programa) {
        formData.append('message', programa); 
    }
    
    // 3. Agregar el archivo (con la clave 'file' que espera tu Django Serializer)
    if (pdfFile) {
    ...
    const response = await fetch('http://localhost:8000/chat/messages/', {
      method: 'POST',
      // <--- ¡ATENCIÓN! BORRAMOS los headers de Content-Type. 
    ...
    // Retornamos la estructura de tramos, inyectando el título de la API en el primero
    return [
      {
        id: 1,
        titulo: tituloApi, // <--- ¡Acá se usa lo que devuelve la API!
...
export default function App() {
  ...
  const handleIniciarSesion = async (e) => {
    ...
    try {
      // ✅ LLAMAMOS A LA FUNCIÓN NUEVA QUE SE CONECTA CON DJANGO
      ...
      if (data && data.length > 0) {
        ...
        // Si falló o no devolvió datos, volvemos al setup
        setPaso('setup');
      }
    } catch (error) {
      console.error("Error en la sesión:", error);
      alert("No se pudo conectar con el servidor de Django. Asegúrate de que esté encendido.");
      setPaso('setup'); // Vuelve a la pantalla inicial para que no quede trabado
    }
  };
